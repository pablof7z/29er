...
pub fn register_relay_selector_runtime(app: &nmp_native_runtime::NmpApp) -> Arc<RelaySelectorProjection> {
...
fn publish_relay_set(tx: &nmp_core::CommandSender, relays: Vec<String>) {
    ...
    let _ = tx.send(ActorCommand::Publish(PublishCommand::UnsignedEvent {
        event,
        correlation_id: Some(crate::dispatch::mint_correlation_id()),
        signer_pubkey: None,
    }));
}
