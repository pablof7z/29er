...
impl Composer {
    ...
    fn current_word(&self) -> String {
        ...
        upto.rsplit(|c: char| c.is_whitespace())
            .next()
            .unwrap_or("")
            .to_string()
    }
    ...
    fn label(m: &GroupMemberRow) -> String {
        m.display_name
            .clone()
            .filter(|n| !n.is_empty())
    ...
    fn refresh_mention(&mut self) {
        match self.mention_query() {
            Some(q) if !self.matches(&q).is_empty() => {
                ...
                if self.mention_sel >= n {
                    self.mention_sel = n.saturating_sub(1);
                }
...
impl Component for Composer {
    fn draw(&mut self, f: &mut Frame, area: Rect) {
        ...
        let chunks = Layout::default()
            ...
            .constraints([Constraint::Min(3), Constraint::Length(outbox_h)])
            .split(area);
        ...
        if outbox_h > 0 {
            f.render_widget(Paragraph::new(outbox_lines), chunks[1]);
        }
        if self.mention_open {
            if let Some(q) = self.mention_query() {
                ...
                if !matches.is_empty() {
                    ...
                    let items: Vec<ListItem> = matches
                        ...
                        .map(|m| {
                            ...
                            ListItem::new(Line::from(vec![
                                Span::styled(Self::label(m), Style::default().fg(ui::TEXT)),
                                Span::styled(admin.to_string(), Style::default().fg(ui::YELLOW)),
                            ]))
                        })
                        .collect();
                    ...
                    self.mention_state
                        .select(Some(self.mention_sel.min(matches.len().saturating_sub(1))));
    ...
    fn handle_event(&mut self, event: &Event) -> Option<Action> {
        ...
        if self.mention_open {
            match key.code {
                ...
                KeyCode::Down => {
                    self.mention_sel = self.mention_sel.saturating_add(1);
                ...
                KeyCode::Tab | KeyCode::Enter => {
                    if let Some(q) = self.mention_query() {
                        ...
                        if let Some(m) = matches
                            .get(self.mention_sel.min(matches.len().saturating_sub(1)))
                            .cloned()
                        {
...
mod tests {
    ...
    fn member(name: &str, pk: &str, admin: bool) -> GroupMemberRow {
        GroupMemberRow {
            pubkey: pk.to_string(),
    ...
    fn enter_emits_send_with_trimmed_text() {
        ...
        c.handle_event(&ch('i'));
        assert!(
            matches!(c.handle_event(&enter()), Some(Action::SendMessage { content: t, .. }) if t == "hi")
        );
    }
    #[test]
